...
if data is not None:
    ...
    with st.container():
        ...
        with col2:
            ...
            staircas_correct = staircase_answers[staircase_answers['trials_2.response'] == 1].value_counts()
            st.write(staircas_correct)
            results.update({'staircase_trials' : [total_s_trial] , staircase_answers.columns[1] : [staircase_answers['CorrectAns'].to_string(index=True)],staircase_answers.columns[1] : [staircase_answers['trials_2.response'].to_string(index=True)] , 'staircase_correct' : [[staircas_correct]]})
